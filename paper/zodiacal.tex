...
circle constraint (candidate must lie within distance $|AB|$ of the $AB$
midpoint) rejects geometrically implausible quads early.
...
\begin{tabular}{lcccc}
\toprule
Scenario & $\delta$ & $\alpha = 3$ & $\alpha = \sqrt{2}$ \\
 & & $B_{\text{eff}}$ (reduction) & $B_{\text{eff}}$ (reduction) \\
\midrule
...
crash-safe build manifest after each cell, and a sequential
\emph{tidy phase} that copies the populated cells into a final
canonical bundle layout. A dedicated \emph{manifest actor} thread
